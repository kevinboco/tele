        ...
        <table class="min-w-[1200px] w-full text-sm">
          ...
          <tbody id="tbody" class="divide-y divide-slate-100 bg-white">
            ...
            <tr>
              ...
              <td class="px-3 py-2 text-right">
                <input type="text" class="prest w-full max-w-[120px] rounded-lg border border-slate-300 px-2 py-1 text-right num" value="0" placeholder="0">
              </td>
              <td class="px-3 py-2">
...
<script>
  ...
  const PREST_KEY = 'prestamos:'+COMPANY_SCOPE;
  ...
  let prestMap = getLS(PREST_KEY); if(!prestMap || typeof prestMap!=='object') prestMap = {};
  ...
  [...tbody.querySelectorAll('tr')].forEach(tr=>{
    ...
    const prest = tr.querySelector('input.prest');
    ...
    if (prestMap[baseName]) prest.value = fmt(toInt(prestMap[baseName]));
    ...
    prest.addEventListener('input', ()=>{ prestMap[baseName] = toInt(prest.value); setLS(PREST_KEY, prestMap); recalc(); });
  });
  ...
  function recalc(){
    ...
    rows.forEach((tr,i)=>{
      ...
      const prest=toInt(tr.querySelector('input.prest').value);
  ...
  // Formatear inputs de préstamos y seg social
  [...tbody.querySelectorAll('input.prest')].forEach(el => {
    el.addEventListener('input', ()=>{ 
      const raw=toInt(el.value); 
      el.value=fmt(raw); 
      const baseName=el.closest('tr').children[0].innerText.trim();
      prestMap[baseName] = raw; 
      setLS(PREST_KEY, prestMap); 
      recalc(); 
    });
  });
  
  [...tbody.querySelectorAll('input.ss')].forEach(el => {
    el.addEventListener('input', ()=>{ 
      const raw=toInt(el.value); 
      el.value=fmt(raw); 
      const baseName=el.closest('tr').children[0].innerText.trim();
