...
<style>
  /* Permite posicionar libremente el modal al arrastrar */
  #modalViajes .modal-dialog.draggable {
    position: fixed;
    margin: 0;              /* evita que el margin recentre */
  }
  /* Quita animación de translate para que no “salte” al empezar a arrastrar */
  #modalViajes.modal.fade .modal-dialog {
    transition: none !important;
  }
  /* Muestra que la cabecera es arrastrable */
...
(function () {
  ...
  function onDown(e){
    // Solo arrastrar con botón izquierdo o toque
    if (e.button !== undefined && e.button !== 0) return;
    isDown = true;

    // Poner el modal en modo “libre”
    dialog.classList.add('draggable');
    // Quitar el centrado por transform
  ...
  // Al abrir el modal, céntralo una vez
  modalEl.addEventListener('shown.bs.modal', () => {
    // Limpia cualquier posición previa
    dialog.classList.remove('draggable');
    dialog.style.left = '';
    dialog.style.top = '';
    dialog.style.transform = ''; // Bootstrap lo centra
  });

  // Eventos para arrastrar desde la cabecera
