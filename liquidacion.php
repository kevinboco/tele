...
/* =======================================================
   ...
   (soporta: completo|medio|extra|carrotanque|siapana)
======================================================= */
...
if (isset($_POST['guardar_clasificacion'])) {
    ...
    if (!in_array($clasif, $allowClasif, true)) { echo "error: clasificación inválida"; exit; }
...
/* =======================================================
   🔹 Cálculo y armado de tablas (clasificación manual)
======================================================= */
$desde = $_GET['desde'];
$hasta = $_GET['hasta'];
...
if ($res) {
    while ($row = $res->fetch_assoc()) {
        ...
        if ($clasifRuta === '') continue;

        switch ($clasifRuta) {
            case 'completo':    $datos[$nombre]["completos"]++; break;
            case 'medio':       $datos[$nombre]["medios"]++; break;
            case 'extra':       $datos[$nombre]["extras"]++; break;
            case 'siapana':     $datos[$nombre]["siapana"]++; break;
            case 'carrotanque': $datos[$nombre]["carrotanques"]++; break;
        }
    }
}
...
<style>
  ...
  .alert-cobro { animation: pulse 2s infinite; border-left: 4px solid #f59e0b; }
  ...
  /* 👇 Columna FALTANTE se oculta/mostrar por JS */
  .col-faltante.hidden-col { display:none !important; }
</style>
</head>
      ...
      <section class="space-y-5">
        ...
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-5">
            ...
            <table class="w-full text-xs">
              ...
              <?php foreach($rutasUnicas as $info): ?>
                <tr class="fila-ruta hover:bg-slate-50"
                  ...
                  <td class="px-2 py-1 whitespace-nowrap text-left"><?= htmlspecialchars($info['ruta']) ?></td>
                  <td class="px-2 py-1 text-center"><?= htmlspecialchars($info['vehiculo']) ?></td>
          ...
          <p class="text-[11px] text-slate-500 mt-2">
            Después de cambiar clasificaciones, vuelve a darle <strong>Filtrar</strong> para recalcular.
          </p>
        </div>
      </section>
          ...
          <table id="tabla_conductores" class="w-full text-sm table-fixed">
            <thead class="bg-blue-600 text-white">
              <tr>
                ...
                <!-- 👇 Faltante (se muestra SOLO si hay pagos parciales > 0) -->
                <th class="px-3 py-2 text-center text-yellow-200 col-faltante hidden-col">Faltante</th>
              </tr>
            </thead>
            ...
            <?php foreach ($datos as $conductor => $viajes): ?>
              <tr data-vehiculo="<?= htmlspecialchars($viajes['vehiculo']) ?>"
                  ...
                  <div class="mensual-info hidden flex-col gap-1">
                    <div class="grid grid-cols-2 gap-1">
                      <div>
                        ...
                        <input type="date" class="fecha-desde w-full rounded border border-gray-300 px-2 py-1 text-xs">
                      </div>
                      <div>
                        <label class="text-xs">Hasta:</label>
                        <input type="date" class="fecha-hasta w-full rounded border border-gray-300 px-2 py-1 text-xs">
                      </div>
                    </div>
                    <input type="number"
                ...
                <!-- Pago parcial (siempre visible) -->
                <td class="px-3 py-2">
                  <input type="number" step="1000" min="0"
                         class="pago-parcial w-full rounded-xl border border-slate-300 px-3 py-2 text-right bg-white outline-none tabular-nums"
                         placeholder="0"
                         oninput="guardarPagoParcial(this); recalcular();">
                </td>

                <!-- Faltante (columna condicional por JS) -->
                <td class="px-3 py-2 col-faltante hidden-col">
                  <input type="text"
                         class="faltante w-full rounded-xl border border-yellow-300 px-3 py-2 text-right bg-yellow-50 text-yellow-800 font-semibold tabular-nums"
                         readonly>
                </td>
        ...
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-5">
          ...
          <div class="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            ...
            <p class="text-xs text-blue-700">
              1. Configura fecha DESDE y HASTA para cada conductor mensual<br>
              2. Haz click en "✅ Registrar Cobro" cuando termines<br>
              3. En la próxima liquidación, el sistema usará automáticamente la última fecha cobrada
            </p>
          </div>
        </div>
      </aside>
...
<script>
  ...
  function filtrarConductores() {
    ...
    if (textoBusqueda === '') {
      ...
      filas.forEach(fila => {
        ...
        if (nombreNormalizado.includes(textoBusqueda)) { fila.style.display = ''; filasVisibles++; }
  ...
  clearBuscar.addEventListener('click', () => { buscadorConductores.value=''; filtrarConductores(); buscadorConductores.focus(); });
  buscadorConductores.addEventListener('keydown', (e) => { if (e.key === 'Escape') { buscadorConductores.value=''; filtrarConductores(); } });
  ...
  // 👇 Pagos parciales por empresa + rango (para que no se mezclen periodos)
  const PAGOS_KEY  = 'pagos_parciales_<?= htmlspecialchars($empresaFiltro) ?>_<?= htmlspecialchars($desde) ?>_<?= htmlspecialchars($hasta) ?>';
  ...
  let pagosParciales  = JSON.parse(localStorage.getItem(PAGOS_KEY)) || {}; // { "Conductor": 123000 }
  ...
  function calcularMeses(desdeStr, hastaStr) {
    ...
    while (true) {
      ...
      if (siguiente <= fin) { meses++; inicio = siguiente; }
      else break;
    }
    return meses;
  }
  ...
  function calcularDiasYMonto(fechaDesdeInput, fechaHastaInput, montoInput, diasSpan, detalle) {
    ...
    if (dDesde > dHasta) {
      ...
      if (detalle) { detalle.textContent = "Error: Fecha desde > hasta"; detalle.classList.remove('hidden'); }
      return 0;
    }
  ...
  function calcularMensual(input) {
    ...
    if (fechaDesdeInput.value && fechaHastaInput.value && montoInput.value) {
      configMensuales[conductor] = { desde: fechaDesdeInput.value, hasta: fechaHastaInput.value, monto: parseFloat(montoInput.value) };
    }
  ...
  function registrarCobro(btn) {
    ...
    const conductor = fila.dataset.conductor;
    const fechaHastaInput = fila.querySelector('.fecha-hasta');
    if (!fechaHastaInput.value) { alert('❌ Debes especificar la fecha HASTA para registrar el cobro'); return; }
    ...
    Object.entries(configMensuales).forEach(([conductor, datos]) => {
      ...
      if (historialCobros[conductor]) {
        ...
        if (ultimoCobro >= nuevoDesde) {
          ...
          if (diffDays > 1) { mensaje = `⚠️ ${conductor}: ${diffDays} días sin cobrar desde ${historialCobros[conductor]}`; tipo = 'warning'; hayAlertas = true; }
        }
      } else {
  ...
  function actualizarListaMensuales() {
    ...
    Object.entries(configMensuales).forEach(([conductor, datos]) => {
      const fila = document.querySelector(`tr[data-conductor="${CSS.escape(conductor)}"]`);
      if (!fila) return;
  ...
  // ✅ Guardar pago parcial por conductor (LocalStorage)
  function guardarPagoParcial(input) {
    const fila = input.closest('tr');
    const conductor = fila.dataset.conductor;
    const valor = Math.max(0, parseFloat(input.value) || 0);

    if (!valor) {
      delete pagosParciales[conductor];
    } else {
      pagosParciales[conductor] = valor;
    }
    localStorage.setItem(PAGOS_KEY, JSON.stringify(pagosParciales));

    actualizarVisibilidadFaltante(); // 👈 clave para tu requisito
  }

  // ✅ Mostrar/ocultar columna "Faltante" SOLO si existe al menos 1 pago parcial > 0
  function actualizarVisibilidadFaltante() {
    const hayAlguno = Object.values(pagosParciales).some(v => (parseFloat(v) || 0) > 0);

    document.querySelectorAll('.col-faltante').forEach(el => {
      if (hayAlguno) el.classList.remove('hidden-col');
  ...
  function recalcular() {
    ...
    filas.forEach(f => {
      ...
      // Pago parcial
      const pago = parseFloat(pagosParciales[conductor]) || 0;
      const inpPago = f.querySelector('.pago-parcial');
      if (inpPago && inpPago !== document.activeElement) {
        inpPago.value = pago ? pago : '';
      }

      // Faltante (si la columna está visible)
      const inpFaltante = f.querySelector('.faltante');
      if (inpFaltante) {
        const faltante = Math.max(0, totalFila - pago);
        inpFaltante.value = formatNumber(faltante);
      }
  ...
  function guardarClasificacionRuta(ruta, vehiculo, clasificacion) {
    ...
    .then(r=>r.text())
    .then(t=>{ if (t.trim() !== 'ok') console.error('Error guardando clasificación:', t); });
  }
  ...
  function aplicarClasificacionMasiva() {
    ...
    if (!patron || !clasif) { alert('Escribe un texto y elige una clasificación.'); return; }
  ...
  function cargarConfiguracion() {
    ...
    // Cargar pagos parciales en inputs
    document.querySelectorAll('#tabla_conductores_body tr').forEach(fila => {
      const conductor = fila.dataset.conductor;
      const inp = fila.querySelector('.pago-parcial');
      if (inp) {
        const v = parseFloat(pagosParciales[conductor]) || 0;
        inp.value = v ? v : '';
      }
    });

    actualizarVisibilidadFaltante();
    actualizarListaMensuales();
    mostrarAlertasCobro();
    recalcular();
  }
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.btn-mensual').forEach(btn => {
      btn.addEventListener('click', function() {
        ...
        if (configMensuales[conductor]) {
          ...
          if (!fechaHastaInput.value) fechaHastaInput.value = RANGO_HASTA;
        }
    ...
    document.querySelectorAll('.fecha-desde, .fecha-hasta, .monto-mensual').forEach(input => {
      input.addEventListener('change', function() { calcularMensual(this); });
    });
    ...
    document.querySelectorAll('.tarjeta-tarifa input').forEach(input => {
      input.addEventListener('change', () => {
        ...
        .then(r=>r.text())
        .then(t=>{ if (t.trim() !== 'ok') console.error('Error guardando tarifa:', t); recalcular(); });
      });
    });

    // Click conductor → viajes
    ...
    // 👇 cuando cambien pagos parciales, mostrar/ocultar columna faltante al vuelo
    document.querySelectorAll('.pago-parcial').forEach(inp => {
      inp.addEventListener('input', () => {
        guardarPagoParcial(inp);
        recalcular();
      });
    ...
    // Estado inicial columna faltante
    actualizarVisibilidadFaltante();
  });
</script>
