...
📌 /manual para registrar viaje manualmente (admin)");
    exit;
}
...
// === NUEVO: flujo manual (admin) ===
// EXACTO: nombre del conductor -> ruta -> fecha -> guardar en tabla viajes (solo esos campos)
if ($text == "/manual") {
    // Iniciamos el flujo manual sin exigir registro
...
elseif (!empty($estado) && !$callback_query) {
    switch ($estado["paso"]) {
        ...
        // Año (parte del flujo agg cuando se elige fecha_manual)
        case "anio":
        ...
        // Paso: pedir salida (agg nueva ruta)
        case "nueva_ruta_salida":
            $estado["salida"] = $text;
            $estado["paso"] = "nueva_ruta_destino";
            enviarMensaje($apiURL, $chat_id, "🏁 Ingresa el *destino* de la nueva ruta:");
            break;

        // Paso: pedir destino (agg nueva ruta)
        ...
        // === casos del flujo MANUAL (exacto: nombre -> ruta -> fecha -> guardar) ===
        case "manual_nombre":
        ...
        case "manual_fecha":
            ...
            if (preg_match('/^\d{4}-\d{2}-\d{2}$/', $fecha_text)) {
                list($y, $m, $d) = explode('-', $fecha_text);
                if (checkdate((int)$m, (int)$d, (int)$y)) {
                    // insertar en la tabla viajes solo: nombre, fecha, ruta
                    $conn = new mysqli("mysql.hostinger.com", "u648222299_keboco5", "Bucaramanga3011", "u648222299_viajes");
                    $nombre_safe = $conn->real_escape_string($estado["manual_nombre"]);
                    $ruta_safe   = $conn->real_escape_string($estado["manual_ruta"]);
                    $fecha_safe  = $conn->real_escape_string($fecha_text);
                    $sql = "INSERT INTO viajes (nombre, fecha, ruta) VALUES ('{$nombre_safe}', '{$fecha_safe}', '{$ruta_safe}')";
                    if ($conn->query($sql) === TRUE) {
                        enviarMensaje($apiURL, $chat_id, "✅ Viaje registrado (modo manual):\n👤 {$estado['manual_nombre']}\n🛣 {$estado['manual_ruta']}\n📅 {$fecha_text}");
                        // limpiar estado
                        if (file_exists($estadoFile)) unlink($estadoFile);
                        $estado = [];
                    } else {
                    ...
                    enviarMensaje($apiURL, $chat_id, "⚠️ Fecha inválida. Usa formato YYYY-MM-DD y una fecha real.");
                }
            } else {
                enviarMensaje($apiURL, $chat_id, "⚠️ Formato incorrecto. Ingresa la fecha así: YYYY-MM-DD (ej: 2025-09-22).");
            }
            break;
