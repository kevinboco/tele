                    ...
                    if ($tabla['nombre'] === $tabla_origen_nombre) {
                        foreach ($tabla['filas'] as $fidx => $fila) {
                            if ($fila['id'] === $id) {
                                ...
                                $fila['costo'] = floatval($fila['valor']);
...
function obtenerDatosParaExportar($conn, $fecha_desde, $fecha_hasta, $empresas_seleccionadas, $extras, $PRESUPUESTO_BASE) {
    $datos = array(); $ids_en_extras = array_column($extras, 'id'); $alertas = array();
    foreach ($empresas_seleccionadas as $empresa_actual) {
        ...
        if (!empty($ids_en_extras)) { $placeholders = implode(',', array_fill(0, count($ids_en_extras), '?')); $sql .= " AND v.id NOT IN ($placeholders)"; foreach ($ids_en_extras as $id) { $params[] = $id; $types .= "i"; } }
...
if (isset($_POST['export_word'])) {
    ...
    <h1>📊 Informe de Viajes</h1><div class="info-filtros"><strong>📅 Período:</strong> <?php echo $fecha_desde ? date('d/m/Y', strtotime($fecha_desde)) : 'Todo'; ?> - <?php echo $fecha_hasta ? date('d/m/Y', strtotime($fecha_hasta)) : 'Todo'; ?><br><strong>🏥 Empresas:</strong> <?php echo implode(', ', $empresas_seleccionadas); ?></div>
...
<?php if (!empty($alertas)): ?><div class="alertas-container"><?php foreach ($alertas as $alerta): ?>
    <div class="alerta-presupuesto"><div class="alerta-mensaje"><span class="alerta-icono">⚠️</span><span class="alerta-texto"><strong><?php echo htmlspecialchars($alerta['empresa']); ?></strong> excede presupuesto <span class="badge-exceso">Total: $ <?php echo number_format($alerta['total'], 0, ',', '.'); ?></span></span></div><button class="btn-ir-tabla" onclick="irATabla('<?php echo htmlspecialchars($alerta['empresa']); ?>')">📍 Ir</button></div>
...
<form method="GET" id="filtroForm"><div class="filtros-card">
    <div class="filtros-row">
        ...
        <div class="filtro-group"><a href="?" class="btn-limpiar">🗑️ Limpiar</a></div>
    </div>
...
<div class="extras-table">
    <div class="extras-header"><h2>⭐ EXTRAS ⭐</h2><form method="POST" style="display:inline"><input type="hidden" name="action" value="limpiar_extras"><button class="btn-limpiar-extras">🗑️ Limpiar todas</button></form></div>
    <div style="overflow-x:auto"><table style="min-width:1200px"><thead><tr><th>#</th><th>Fecha</th><th>Conductor</th><th>Cédula</th><th>Ruta</th><th>Tipo</th><th>Empresa Origen</th><th>Clasificación</th><th>Valor</th><th>Acumulado</th><th>Acción</th></tr></thead><tbody>
    <?php $idx_extra=0; foreach($extras_con_acumulado as $extra): $idx_extra++; ?>
        <tr><td style="text-align:center"><?php echo $idx_extra; ?></td><td><?php echo date('d/m/Y', strtotime($extra['data']['fecha'])); ?></td><td><strong><?php echo htmlspecialchars($extra['data']['nombre'] ?? '-'); ?></strong></td><td><?php echo htmlspecialchars($extra['data']['cedula'] ?? '-'); ?></td><td class="ruta-cell"><?php echo htmlspecialchars($extra['data']['ruta'] ?? '-'); ?></td><td><?php echo htmlspecialchars($extra['data']['tipo_vehiculo'] ?? '-'); ?></td><td><?php echo htmlspecialchars($extra['data']['empresa'] ?? ($extra['data']['empresa_origen'] ?? '-')); ?></td><td><?php echo htmlspecialchars($extra['data']['clasificacion'] ?? '-'); ?></td><td class="costo">$ <?php echo number_format($extra['data']['costo'], 0, ',', '.'); ?></td><td class="acumulado">$ <?php echo number_format($extra['acumulado'], 0, ',', '.'); ?></td><td><form method="POST" style="display:inline"><input type="hidden" name="action" value="eliminar_extra"><input type="hidden" name="extra_index" value="<?php echo $extra['index']; ?>"><button class="btn-eliminar-extra">✖</button></form></td></tr>
...
if (!empty($empresas_seleccionadas)) {
    foreach ($empresas_seleccionadas as $empresa_actual) {
        ...
        if (!isset($datos_empresas[$empresa_actual])) { ?><div class="empresa-table" id="<?php echo $empresa_anchor; ?>"><div class="table-header"><h2>🏥 <?php echo htmlspecialchars($empresa_actual); ?></h2></div><div class="sin-datos">📭 No hay viajes</div></div><?php continue; }
        ...
        if ($data['tipo'] === 'multiple'):
            ...
            <div class="empresa-table" id="<?php echo $empresa_anchor; ?>">
                ...
                <?php foreach ($data['tablas'] as $tab_idx => $tabla): 
                    ...
                    <div class="sub-table-wrapper">
                        <div class="table-header-conductor"><h3>👤 <?php echo htmlspecialchars($conductor_nombre); ?></h3><div class="acciones-header"><select class="destino-select" id="destino_<?php echo $empresa_id; ?>"><?php foreach($destinos_disponibles as $dk=>$dv): if($dk==($empresa_actual.'||'.$conductor_nombre)) continue; ?><option value="<?php echo htmlspecialchars($dk); ?>"><?php echo htmlspecialchars($dv); ?></option><?php endforeach; ?></select><button class="btn-mover-extras" onclick="moverSeleccionados('<?php echo $empresa_id; ?>')" id="btn-mover-<?php echo $empresa_id; ?>">➡️ Mover</button></div></div>
                        ...
                        <div style="overflow-x:auto"><table style="min-width:1000px"><thead><tr><th><input type="checkbox" id="select-all-<?php echo $empresa_id; ?>" onchange="toggleSeleccionarTodos(this,'<?php echo $empresa_id; ?>')"></th><th>#</th><th>Fecha</th><th>Conductor</th><th>Cédula</th><th>Ruta</th><th>Tipo</th><th>Clasificación</th><th>Valor</th><th>Acumulado</th></tr></thead><tbody id="tbody-<?php echo $empresa_id; ?>"><?php $contador=0;$acum=0;foreach($rows_data as $row):$contador++;$acum+=$row['costo']; ?><tr data-ruta="<?php echo strtolower($row['ruta']??''); ?>"><td class="checkbox-col"><input type="checkbox" class="fila-check-<?php echo $empresa_id; ?>" value="<?php echo $row['id']; ?>"></td><td style="text-align:center"><?php echo $contador; ?></td><td><?php echo date('d/m/Y',strtotime($row['fecha'])); ?></td><td><strong><?php echo htmlspecialchars($row['nombre']??'-'); ?></strong></td><td><?php echo htmlspecialchars($row['cedula']??'-'); ?></td><td class="ruta-cell"><?php echo htmlspecialchars($row['ruta']??'-'); ?></td><td><?php echo htmlspecialchars($row['tipo_vehiculo']??'-'); ?></td><td><?php echo htmlspecialchars($row['clasificacion']??'-'); ?></td><td class="costo">$ <?php echo number_format($row['costo'],0,',','.'); ?></td><td class="acumulado">$ <?php echo number_format($acum,0,',','.'); ?></td></tr><?php endforeach; ?></tbody></table></div>
                        <div style="background:#e8f0fe;padding:12px 20px;display:flex;align-items:center;justify-content:flex-end;gap:12px;flex-wrap:wrap"><strong>TOTAL <?php echo htmlspecialchars($conductor_nombre); ?>:</strong><?php if($es_manual): ?><span style="font-size:11px;color:#999;text-decoration:line-through">$ <?php echo number_format($total_calculado,0,',','.'); ?></span><span style="color:#e65100">➜</span><?php endif; ?><span style="font-weight:700;font-size:14px;color:#1a73e8">$ <?php echo number_format($total_efectivo,0,',','.'); ?></span><form method="POST" style="display:inline-flex;align-items:center;gap:6px"><input type="hidden" name="action" value="guardar_total_manual"><input type="hidden" name="total_key" value="<?php echo htmlspecialchars($key_tabla); ?>"><input type="number" name="total_valor" value="<?php echo $total_efectivo; ?>" step="1" style="width:110px;padding:4px 8px;border:1px solid #1a73e8;border-radius:6px;text-align:right;font-weight:600;font-size:12px"><button class="btn-guardar-total">💾</button></form></div>
                    </div>
                    <script>(function(){const eid='<?php echo $empresa_id; ?>';const cbs=document.querySelectorAll('.fila-check-'+eid);let dragging=false;cbs.forEach(cb=>{cb.addEventListener('mousedown',()=>{dragging=true;cb.checked=!cb.checked;updateSelectAllCheckbox(eid);actualizarBotonMover(eid)});cb.addEventListener('mouseenter',()=>{if(dragging){cb.checked=!cb.checked;updateSelectAllCheckbox(eid);actualizarBotonMover(eid)}})});document.addEventListener('mouseup',()=>{dragging=false});let lastCb=null;cbs.forEach(cb=>{cb.addEventListener('click',function(e){if(e.shiftKey&&lastCb){const arr=Array.from(cbs);const s=Math.min(arr.indexOf(this),arr.indexOf(lastCb));const en=Math.max(arr.indexOf(this),arr.indexOf(lastCb));for(let i=s;i<=en;i++)arr[i].checked=this.checked;updateSelectAllCheckbox(eid);actualizarBotonMover(eid)}lastCb=this})});window.updateSelectAllCheckbox=function(id){const sa=document.getElementById('select-all-'+id);if(!sa)return;const cs=document.querySelectorAll('.fila-check-'+id);const all=Array.from(cs).every(c=>c.checked);const some=Array.from(cs).some(c=>c.checked);sa.checked=all;sa.indeterminate=!all&&some};window.actualizarBotonMover=function(id){const cs=document.querySelectorAll('.fila-check-'+id);const chk=Array.from(cs).filter(c=>c.checked);const btn=document.getElementById('btn-mover-'+id);if(btn){btn.disabled=chk.length===0;btn.innerHTML=chk.length>0?'➡️ Mover '+chk.length+' a destino':'➡️ Mover'}};window.toggleSeleccionarTodos=function(cb,id){document.querySelectorAll('.fila-check-'+id).forEach(c=>c.checked=cb.checked);updateSelectAllCheckbox(id);actualizarBotonMover(id)};window.moverSeleccionados=function(id){const cs=document.querySelectorAll('.fila-check-'+id);const ids=Array.from(cs).filter(c=>c.checked).map(c=>c.value);if(ids.length===0)return;document.getElementById('ids-'+id).value=ids.join(',');document.getElementById('destino_hidden_'+id).value=document.getElementById('destino_'+id).value;document.getElementById('form-'+id).submit()};cbs.forEach(cb=>cb.addEventListener('change',()=>{updateSelectAllCheckbox(eid);actualizarBotonMover(eid)}));actualizarBotonMover(eid)})();</script>
                <?php endforeach; ?>
            ...
            <?php if($es_nazareth): ?><script>window.seleccionarPorRuta=function(eid){const inp=document.getElementById('buscar_ruta_'+eid);const txt=inp.value.trim().toLowerCase();if(!txt){alert('Escribe una palabra');return}const subs=document.querySelectorAll('[id^="tbody-'+eid+'_c"]');let sel=0;subs.forEach(tb=>{tb.querySelectorAll('tr').forEach(fila=>{const cel=fila.querySelector('.ruta-cell');if(cel){const cb=fila.querySelector('input[type="checkbox"]');if(cb&&cel.textContent.toLowerCase().includes(txt)){cb.checked=true;sel++}}})});subs.forEach(tb=>{const sid=tb.id.replace('tbody-','');if(typeof updateSelectAllCheckbox==='function')updateSelectAllCheckbox(sid);if(typeof actualizarBotonMover==='function')actualizarBotonMover(sid)});if(sel===0)alert('No se encontraron rutas con "'+txt+'"')};</script><?php endif; ?>
        <?php else: 
            ...
            if(empty($rows_data)){?><div class="empresa-table" id="<?php echo $empresa_anchor; ?>"><div class="table-header"><h2>🏥 <?php echo htmlspecialchars($empresa_actual); ?></h2></div><div class="sin-datos">📭 No hay viajes</div></div><?php continue;} ?>
            <div class="empresa-table" id="<?php echo $empresa_anchor; ?>">
                <div class="table-header"><h2>🏥 <?php echo htmlspecialchars($empresa_actual); ?><?php if(!empty($nombres_seleccionados)): ?> <span class="badge-exceso" style="background:#1a73e8">✏️ Nombres cambiados</span><?php endif; ?></h2><div style="display:flex;gap:15px;align-items:center;flex-wrap:wrap"><?php if($es_nazareth): ?><div class="busqueda-ruta"><input type="text" id="buscar_ruta_<?php echo $empresa_id; ?>" placeholder="Ej: Riohacha, Maicao..." autocomplete="off"><button type="button" onclick="seleccionarPorRuta('<?php echo $empresa_id; ?>')">🔍</button></div><?php endif; ?><div class="drag-instruction">🖱️ Arrastra sobre checkboxes</div><div class="acciones-header"><select class="destino-select" id="destino_<?php echo $empresa_id; ?>"><?php foreach($destinos_disponibles as $dk=>$dv): if($dk==$empresa_actual) continue; ?><option value="<?php echo htmlspecialchars($dk); ?>"><?php echo htmlspecialchars($dv); ?></option><?php endforeach; ?></select><button class="btn-mover-extras" onclick="moverSeleccionados('<?php echo $empresa_id; ?>')" id="btn-mover-<?php echo $empresa_id; ?>">➡️ Mover</button></div></div></div>
                <div class="cambio-conductores-section"><div class="filtro-group" style="flex:2;min-width:300px"><label>👤 Buscar o escribir conductor (máx. 2) - Enter para agregar</label><div class="autocomplete-wrapper"><input type="text" id="input_conductor_<?php echo $empresa_id; ?>" placeholder="Escribe y selecciona o presiona Enter..." autocomplete="off" onkeyup="buscarConductores('<?php echo $empresa_id; ?>')" onfocus="buscarConductores('<?php echo $empresa_id; ?>')" onkeydown="manejarTeclaConductor(event,'<?php echo $empresa_id; ?>')"><div class="autocomplete-list" id="autocomplete_<?php echo $empresa_id; ?>"></div></div><div class="input-hint">💡 Selecciona de la lista o presiona Enter para nombre libre</div><div class="tags-conductores" id="tags_<?php echo $empresa_id; ?>"><?php foreach($nombres_seleccionados as $idx=>$nombre): ?><span class="tag-conductor"><?php echo htmlspecialchars($nombre); ?> <span class="remove-tag" onclick="removerConductor('<?php echo $empresa_id; ?>',<?php echo $idx; ?>)">✕</span></span><?php endforeach; ?></div></div><div class="filtro-group" style="flex:0 0 auto"><button class="btn-cambiar-conductores" id="btn_cambiar_<?php echo $empresa_id; ?>" onclick="cambiarConductores('<?php echo $empresa_id; ?>','<?php echo htmlspecialchars($empresa_actual); ?>')" <?php echo empty($nombres_seleccionados)?'disabled':''; ?>>✏️ Cambiar Conductores</button></div></div>
                <form method="POST" id="form-<?php echo $empresa_id; ?>"><input type="hidden" name="action" value="mover_viajes"><input type="hidden" name="empresa_origen" value="<?php echo htmlspecialchars($empresa_actual); ?>"><input type="hidden" name="ids_seleccionados" id="ids-<?php echo $empresa_id; ?>"><input type="hidden" name="destino" id="destino_hidden_<?php echo $empresa_id; ?>"></form>
                <div style="overflow-x:auto"><table style="min-width:1000px"><thead><tr><th><input type="checkbox" id="select-all-<?php echo $empresa_id; ?>" onchange="toggleSeleccionarTodos(this,'<?php echo $empresa_id; ?>')"></th><th>#</th><th>Fecha</th><th>Conductor</th><th>Cédula</th><th>Ruta</th><th>Tipo</th><th>Clasificación</th><th>Valor</th><th>Acumulado</th></tr></thead><tbody id="tbody-<?php echo $empresa_id; ?>"><?php $contador=0;foreach($rows_data as $row):$contador++; ?><tr data-ruta="<?php echo strtolower($row['ruta']??''); ?>"><td class="checkbox-col"><input type="checkbox" class="fila-check-<?php echo $empresa_id; ?>" value="<?php echo $row['id']; ?>"></td><td style="text-align:center"><?php echo $contador; ?></td><td><?php echo date('d/m/Y',strtotime($row['fecha'])); ?></td><td><strong><?php echo htmlspecialchars($row['nombre']??'-'); ?></strong></td><td><?php echo htmlspecialchars($row['cedula']??'-'); ?></td><td class="ruta-cell"><?php echo htmlspecialchars($row['ruta']??'-'); ?></td><td><?php echo htmlspecialchars($row['tipo_vehiculo']??'-'); ?></td><td><?php echo htmlspecialchars($row['clasificacion']??'-'); ?></td><td class="costo">$ <?php echo number_format($row['costo'],0,',','.'); ?></td><td class="acumulado">$ <?php echo number_format($row['acumulado'],0,',','.'); ?></td></tr><?php endforeach; ?></tbody></table></div>
...
<?php foreach ($tablas_personalizadas_data as $tpidx => $tp): 
    ...
    <div class="empresa-table">
        <div class="table-header-personal"><h3>📋 <?php echo htmlspecialchars($tp_nombre); ?></h3><div class="acciones-header"><button class="btn-agregar-fila" onclick="mostrarModalAgregarFila('<?php echo htmlspecialchars($tp_nombre); ?>')">➕ Agregar viaje</button><select class="destino-select" id="destino_<?php echo $pers_id; ?>"><?php foreach($destinos_disponibles as $dk=>$dv): if($dk=='__personal__'.$tp_nombre) continue; ?><option value="<?php echo htmlspecialchars($dk); ?>"><?php echo htmlspecialchars($dv); ?></option><?php endforeach; ?></select><button class="btn-mover-extras" onclick="moverSeleccionados('<?php echo $pers_id; ?>')" id="btn-mover-<?php echo $pers_id; ?>">➡️ Mover</button><form method="POST" style="display:inline"><input type="hidden" name="action" value="eliminar_tabla_personalizada"><input type="hidden" name="tabla_nombre" value="<?php echo htmlspecialchars($tp_nombre); ?>"><button class="btn-eliminar-tabla" onclick="return confirm('¿Eliminar esta tabla?')">🗑️ Eliminar tabla</button></form></div></div>
    ...
    <script>(function(){const eid='<?php echo $pers_id; ?>';const cbs=document.querySelectorAll('.fila-check-'+eid);let dragging=false;cbs.forEach(cb=>{cb.addEventListener('mousedown',()=>{dragging=true;cb.checked=!cb.checked;updateSelectAllCheckbox(eid);actualizarBotonMover(eid)});cb.addEventListener('mouseenter',()=>{if(dragging){cb.checked=!cb.checked;updateSelectAllCheckbox(eid);actualizarBotonMover(eid)}})});document.addEventListener('mouseup',()=>{dragging=false});let lastCb=null;cbs.forEach(cb=>{cb.addEventListener('click',function(e){if(e.shiftKey&&lastCb){const arr=Array.from(cbs);const s=Math.min(arr.indexOf(this),arr.indexOf(lastCb));const en=Math.max(arr.indexOf(this),arr.indexOf(lastCb));for(let i=s;i<=en;i++)arr[i].checked=this.checked;updateSelectAllCheckbox(eid);actualizarBotonMover(eid)}lastCb=this})});window.updateSelectAllCheckbox=function(id){const sa=document.getElementById('select-all-'+id);if(!sa)return;const cs=document.querySelectorAll('.fila-check-'+id);const all=Array.from(cs).every(c=>c.checked);const some=Array.from(cs).some(c=>c.checked);sa.checked=all;sa.indeterminate=!all&&some};window.actualizarBotonMover=function(id){const cs=document.querySelectorAll('.fila-check-'+id);const chk=Array.from(cs).filter(c=>c.checked);const btn=document.getElementById('btn-mover-'+id);if(btn){btn.disabled=chk.length===0;btn.innerHTML=chk.length>0?'➡️ Mover '+chk.length+' a destino':'➡️ Mover'}};window.toggleSeleccionarTodos=function(cb,id){document.querySelectorAll('.fila-check-'+id).forEach(c=>c.checked=cb.checked);updateSelectAllCheckbox(id);actualizarBotonMover(id)};window.moverSeleccionados=function(id){const cs=document.querySelectorAll('.fila-check-'+id);const ids=Array.from(cs).filter(c=>c.checked).map(c=>c.value);if(ids.length===0)return;document.getElementById('ids-'+id).value=ids.join(',');document.getElementById('destino_hidden_'+id).value=document.getElementById('destino_'+id).value;document.getElementById('form-'+id).submit()};cbs.forEach(cb=>cb.addEventListener('change',()=>{updateSelectAllCheckbox(eid);actualizarBotonMover(eid)}));actualizarBotonMover(eid)})();</script>
<?php endforeach; ?>
...
function buscarConductores(eid){const inp=document.getElementById('input_conductor_'+eid);const lista=document.getElementById('autocomplete_'+eid);const texto=inp.value.trim();if(!texto){lista.style.display='none';return}const coincidencias=todosLosConductores.filter(c=>c.toLowerCase().includes(texto.toLowerCase()));if(coincidencias.length===0){lista.innerHTML='<div style="padding:10px;color:#999">Sin coincidencias - Presiona Enter para agregar "'+texto+'"</div>';lista.style.display='block';return}lista.innerHTML=coincidencias.map(c=>'<div onclick="seleccionarConductor(\''+eid+'\',\''+c.replace(/'/g,"\\'")+'\')">'+c+'</div>').join('')+'<div style="padding:8px 15px;background:#f0f0f0;color:#666;font-size:11px;border-top:1px solid #ddd">💡 O presiona Enter para agregar: <strong>'+texto+'</strong></div>';lista.style.display='block'}
function manejarTeclaConductor(event,eid){if(event.key==='Enter'){event.preventDefault();const inp=document.getElementById('input_conductor_'+eid);const texto=inp.value.trim();if(!texto)return;agregarConductorLibre(eid,texto);inp.value='';document.getElementById('autocomplete_'+eid).style.display='none'}}
function agregarConductorLibre(eid,conductor){if(!conductoresSeleccionados[eid])conductoresSeleccionados[eid]=[];if(conductoresSeleccionados[eid].length>=2){alert('Máximo 2 conductores');return}if(conductoresSeleccionados[eid].includes(conductor)){alert('Ya seleccionado');return}conductoresSeleccionados[eid].push(conductor);actualizarTags(eid);actualizarBotonCambiar(eid)}
...
function actualizarTags(eid){const tc=document.getElementById('tags_'+eid);if(!tc)return;tc.innerHTML=(conductoresSeleccionados[eid]||[]).map((c,i)=>'<span class="tag-conductor">'+c+' <span class="remove-tag" onclick="removerConductor(\''+eid+'\','+i+')">✕</span></span>').join('')}
function actualizarBotonCambiar(eid){const btn=document.getElementById('btn_cambiar_'+eid);if(!btn)return;btn.disabled=(conductoresSeleccionados[eid]||[]).length===0}
function cambiarConductores(eid,empresa){const conductores=conductoresSeleccionados[eid]||[];if(conductores.length===0){alert('Selecciona al menos un conductor');return}const form=document.createElement('form');form.method='POST';form.style.display='none';form.innerHTML='<input type="hidden" name="action" value="cambiar_conductores"><input type="hidden" name="empresa_cambio" value="'+empresa+'">'+conductores.map(c=>'<input type="hidden" name="nombres_conductores[]" value="'+c+'">').join('');document.body.appendChild(form);form.submit()}
function seleccionarTodas(sel){document.querySelectorAll('#empresasGrid input[type="checkbox"]').forEach(cb=>cb.checked=sel);document.getElementById('filtroForm').submit()}
document.querySelectorAll('#empresasGrid input[type="checkbox"]').forEach(cb=>cb.addEventListener('change',()=>document.getElementById('filtroForm').submit()));
function irATabla(empresa){const el=document.getElementById('tabla_'+empresa.replace(/[^a-zA-Z0-9]/g,'_'));if(el){el.scrollIntoView({behavior:'smooth',block:'start'});el.style.boxShadow='0 0 0 3px #ff9800, 0 4px 12px rgba(0,0,0,0.15)';setTimeout(()=>el.style.boxShadow='',2000)}}
document.addEventListener('click',function(e){document.querySelectorAll('.autocomplete-list').forEach(lista=>{if(!lista.parentElement.contains(e.target))lista.style.display='none'})});
