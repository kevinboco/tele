<?php
// nav_minirail_con_efecto.php — Riel vertical con botón circular + imágenes + magnificación hover
?>
<style>
:root{
  --rail-w: 64px;
  --bg: #0a0a0a; --br:#1f1f1f; --btn:#111;
...
/* Riel */
.mini-rail{
  position:fixed; left:10px; top:84px; height:calc(100vh - 100px); width:var(--rail-w);
...
<nav id="miniRail" class="mini-rail" aria-label="Navegación rápida">
  ...
  <a class="rail-item" href="informe.php" title="Informe de viajes">
    <img src="privado/informe.png" alt="Informe de viajes">
  </a>
  <a class="rail-item" href="https://asociacion.asociaciondetransportistaszonanorte.io/tele/liquidacion.php?desde=2025-09-29&hasta=2025-10-12&empresa=Hospital" title="Liquidación">
    <img src="privado/liquidacion.png" alt="Liquidación">
  </a>
  <a class="rail-item" href="https://asociacion.asociaciondetransportistaszonanorte.io/tele/prueba.php?view=graph" title="Mapa préstamos (D3)">
    <img src="privado/mapa_prestamos.png" alt="Mapa préstamos">
  </a>
  <a class="rail-item" href="https://asociacion.asociaciondetransportistaszonanorte.io/tele/admin_prestamos.php?view=cards" title="Editar préstamos">
    <img src="privado/editar_prestamos.png" alt="Editar préstamos">
  </a>
</nav>
...
(function(){
  ...
  /* ===== Efecto de magnificación vertical (hover cerca) ===== */
  const items = Array.from(rail.querySelectorAll('.rail-item'));
  rail.addEventListener('mousemove', (e) => {
    const maxDistance = 140;   // rango de influencia (px)
    const base = 48;           // tamaño base (px)
    ...
    items.forEach(item => {
      ...
      if (d < maxDistance){
        const scale = 1 + (1 - d/maxDistance)*0.35; // 1 → 1.35
        const size = base * scale;
        item.style.width = size + 'px';
        item.style.height = size + 'px';
        item.style.zIndex = 1000 - d; // el más cercano adelante
      } else {
