<?php
/*********************************************************
 * admin_prestamos.php — CRUD + Tarjetas + Visual 3-nodos con pago por selección
 * - Normaliza nombres (no distingue mayúsc/minúsc)
 * - Interés: 10% desde el día 1 + 10% por cada mes
 * - Visual: cada deudor muestra valor, fecha, interés y total
 * - Botón “Préstamo pagado” SOLO marca los NODOS seleccionados (por id exactos)
 *********************************************************/
...
function ensure_schema(){
  ...
  $st=$c->prepare("SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=? AND TABLE_NAME='prestamos' AND COLUMN_NAME='pagado'"); $st->bind_param("s",$db); $st->execute(); $st->bind_result($cnt); $st->fetch(); $st->close(); if((int)$cnt===0) $need_pagado=1;
  $st=$c->prepare("SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=? AND TABLE_NAME='prestamos' AND COLUMN_NAME='pagado_at'"); $st->bind_param("s",$db); $st->execute(); $st->bind_result($cnt2); $st->fetch(); $st->close(); if((int)$cnt2===0) $need_pagado_at=1;
...
// ===== Acción: marcar pagado SÓLO ids seleccionados =====
if ($action==='mark_paid' && $_SERVER['REQUEST_METHOD']==='POST'){
  $idsPacks = $_POST['ids'] ?? [];                // cada valor: "12,18,23" (ids de ese nodo)
  if (!is_array($idsPacks)) $idsPacks = [];
  // aplanar a un array de enteros únicos
  $allIds = [];
  ...
  if (!empty($idList)){
    $c=db();
    // dividir en chunks para armar placeholders variables
...
<?php
    ...
    // NOTA: añadimos GROUP_CONCAT(id) AS ids para poder marcar SOLO esos ids
    $sql = "
      SELECT 
        ...
        GROUP_CONCAT(id ORDER BY fecha,id) AS ids,
    ...
    $labelFor = fn($pkey, $fallback) => mbtitle($fallback);
?>
    <div class="card" style="margin-bottom:16px">
      <form class="toolbar" method="get">
    ...
    ?>
      <form class="group" method="post" action="?action=mark_paid">
        ...
        <div class="title" style="margin:6px 10px 10px">Prestamista: <?= h(mbtitle($prestLabel)) ?></div>
        ...
        <div class="svgwrap">
          <svg width="1320" height="<?= $height ?>" viewBox="0 0 1320 <?= $height ?>" xmlns="http://www.w3.org/2000/svg">
            ...
            <text class="txt" x="<?= $xL-80 ?>" y="<?= $prestY+40 ?>"><tspan font-weight="800"><?= h(mbtitle($prestLabel)) ?></tspan></text>
            ...
            <?php
              ...
              foreach($rows as $r):
                ...
                $idsStr = preg_replace('/[^0-9,]/','',(string)$r['ids']); // ids CSV sanos
            ?>
              ...
              <!-- Checkbox (nodo 2) con ids exactos -->
