 ...
 * - Vista previa de pagos con reestructuración automática
        ...
        if ($restante >= $p['total']) {
            ...
            if ($restante >= $p['interes_total']) {
                ...
                $resultado[] = [
                    ...
                    'mensaje' => "Se pagan intereses ($" . money($interesPagado) . ") y se abonan $" . money($abonoCapital) . " a capital. Nuevo préstamo: $" . money($nuevoCapital)
                ];
                $prestamosAReestructurar[] = [
                    'original_id' => $p['id'],
                    'nuevo_capital' => $nuevoCapital,
                ...
                // No alcanza ni para intereses (caso raro, pero manejarlo)
                $resultado[] = [
                    'id' => $p['id'],
                    'deudor' => $p['deudor'],
                    'monto_original' => $p['monto'],
...
if ($action === 'confirmar_pago' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        foreach ($reestructurar as $r) {
            ...
            $nuevoId = 0; // Placeholder, se actualizará después
            ...
            // Actualizar la nota del original con el nuevo ID
...
// ===== CRUD básico (tus funciones existentes) =====
...
// ... (aquí van tus funciones edit, delete existentes) ...
